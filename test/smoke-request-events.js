...
const location = '../out/provider/request-events';
assert.ok(fs.existsSync(path.join(__dirname, location+'.js')), 'request events adapter must exist');
...
await check('real summary send retains payload and links IDs',async()=>{const {makeFoldSummarize,buildFoldSummaryRequest}=require('../out/provider/fold-summarize');const messages=[{role:'user',content:'SECRET'}];let sent;const summarize=makeFoldSummarize({completeChat:async(req)=>{sent=JSON.stringify(req);return 'SECRET RESPONSE';}},'model',undefined,undefined,id);assert.equal(await summarize(messages),'SECRET RESPONSE');assert.equal(sent,JSON.stringify(buildFoldSummaryRequest('model',messages)));const rows=api.readErrorSummary({requestId:id,includeInfo:true});const summary=rows.filter(r=>r.requestKind==='summary');assert.ok(summary.some(r=>r.eventCode==='SEND_ATTEMPT'));assert.ok(summary.some(r=>r.eventCode==='USAGE_UNAVAILABLE'));assert.ok(summary.every(r=>r.parentRequestId===id && r.requestId!==id));assert.ok(!JSON.stringify(rows).includes('SECRET'));});
await check('summary failure recorded',async()=>{const {makeFoldSummarize}=require('../out/provider/fold-summarize');assert.equal(await makeFoldSummarize({completeChat:async()=>{throw Error('SECRET');}},'model',undefined,undefined,id)([]),'');assert.ok(api.readErrorSummary({requestId:id}).some(r=>r.requestKind==='summary'&&r.eventCode==='REQUEST_SEND_FAILED'));});
...
for(const mode of ['usage','no-usage','failure']) await check('real stream '+mode,async()=>{
 const {DeepSeekClient}=require('../out/client/core'); const {streamChatCompletion}=require('../out/provider/stream');
 const request={model:'model',stream:true,messages:[{role:'user',content:'PRIVATE BODY'}]};
 const requestId=api.newRequestId();let body; const originalFetch=global.fetch;
...
await check('workspace identity stable and unknown stays empty',()=>{const hooks=require('../out/provider/chat-hooks');assert.equal(hooks.workspaceIdentity(),'');workspace.workspaceFolders=[{uri:{toString:()=> 'file:///fixture-a'}}];const a=hooks.workspaceIdentity();assert.ok(a);assert.equal(a,hooks.workspaceIdentity());workspace.workspaceFolders=[{uri:{toString:()=> 'file:///fixture-b'}}];assert.notEqual(a,hooks.workspaceIdentity());delete workspace.workspaceFolders;});
