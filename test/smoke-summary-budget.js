...
const chunks=()=>Array.from({length:4},(_,i)=>user('BLOCK'+i+' '+String(i).repeat(12000)));
...
test('1.3M 多消息切块；每次受限；原文逐字不变',async()=>{
 const msgs=Array.from({length:130},(_,i)=>user('ID'+i+' '+ 'x'.repeat(50000))),before=JSON.stringify(msgs);
...
test('超线 prefix 不能覆盖已切块 foldMsgs；固定 system 保留',async()=>{
 const sys={role:'system',content:'must preserve'},msgs=chunks(),prefix=[sys,...msgs,user('EXCLUDE_CURRENT '+'z'.repeat(40000))],x=fixture();
...
test('单个巨型消息不能截字；零调用',async()=>{
 const msgs=[user('x'.repeat(40000))],before=JSON.stringify(msgs),x=fixture();
...
function pair(size=5600){return [{role:'assistant',content:'',tool_calls:[{id:'a',type:'function',function:{name:'run',arguments:'{}'}},{id:'b',type:'function',function:{name:'run',arguments:'{}'}}]},{role:'tool',tool_call_id:'a',content:'A'.repeat(size)},{role:'tool',tool_call_id:'b',content:'B'.repeat(size)}]}
test('assistant 与所有 tool 始终同组同请求',async()=>{
 const group=pair(),msgs=[user('x'.repeat(12000)),...group,user('y'.repeat(12000))],x=fixture();assert.ok(await x.f(msgs));assert.ok(x.seen.length>1);
...
test('单巨型工具组拒绝，不拆工具回复',async()=>{
 const x=fixture();assert.equal(await x.f(pair(16000)),'');assert.equal(x.seen.length,0);assert.equal(x.f.lastDiagnostic.reason,'budget');
});
...
test('实际 tools 超预算零调用',async()=>{
 const tools=[{type:'function',function:{name:'run',parameters:{description:'x'.repeat(32000)}}}],x=fixture(undefined,small,undefined,tools);
...
test('调用上限 12，不返回部分内容',async()=>{
 const msgs=Array.from({length:13},()=>user('x'.repeat(20000))),x=fixture();
...
test('缩减层最多一层且每次仍受预算',async()=>{
 const x=fixture(async(r,n)=>n<=4?'s'.repeat(16000):'reduced '+n);
...
test('第二层仍过大返回空，不无限再总结',async()=>{
 const x=fixture(async()=> 's'.repeat(16000));
...
test('映射为user的宿主前缀在每个分块保留且不列入摘要目标',async()=>{
 const host=user('HOST RULE KEEP'),msgs=chunks(),x=fixture();assert.ok(await x.f(msgs,{prefixMessages:[host,...msgs,user('CURRENT '+ 'z'.repeat(40000))],protectedPrefixCount:1}));assert.ok(x.seen.length>1);
